...
lock = Lock()
def ensemble_add(json_idx, image_dir, center_json_dir, retina_json_dir, yolov4_json_dir, \
    ...
    if os.path.isfile(center_json_file_path):
        ...
        #center_bbox_list = filter(lambda bbox: bbox[6] >= 0.1, center_bbox_list)
    ...
    if os.path.isfile(retina_json_file_path):
        ...
        #retina_bbox_list = filter(lambda bbox: bbox[6] >= 0.1, retina_bbox_list)
    ...
    if os.path.isfile(yolov4_json_file_path):
        ...
        #yolov4_bbox_list = filter(lambda bbox: bbox[6] >= 0.01, yolov4_bbox_list)
    ...
    for center_bbox in list(center_bbox_list):
        ...
        for candidate_bbox in candidate_bbox_list:
            if candidate_bbox != None:
                num_hit = num_hit + 1
                hit_score = hit_score + candidate_bbox[6]

            if num_hit == 1 and hit_score >= threshold['1hit']:
                ensemble_bbox_list.append(candidate_bbox_list)
                break
            if num_hit == 2 and hit_score >= threshold['2hit']:
                ensemble_bbox_list.append(candidate_bbox_list)
                break
            if num_hit == 3 and hit_score >= threshold['3hit']:
                ensemble_bbox_list.append(candidate_bbox_list)
                break
        assert num_hit <= 3, "num_hit should be less than or equal to 3"
...
def get_ensemble_result_on_test_and_fn(model_type, test_and_fn_list = ['test', 'fn']):
    ...
    if select_bbox_add_score:
        threshold = {'3hit': 0.2, '2hit': 0.4, '1hit': 0.8}
    else:
    ...
    for test_and_fn, ground_truth_image_dir, ground_truth_label_dir, centernet2_label_dir, retinanet_label_dir, yolov4_label_dir in \
        ...
        if concurrent_ensemble:
            ...
            for json_idx, json_file_name in enumerate(os.listdir(ground_truth_label_dir)):
                if select_bbox_add_score:
                    mp_pool.apply_async(ensemble_add, args=(json_idx, ground_truth_image_dir, centernet2_label_dir, retinanet_label_dir, yolov4_label_dir, \
            ...
            for json_idx, json_file_name in enumerate(os.listdir(ground_truth_label_dir)):
                if select_bbox_add_score:
                    ensemble_add(json_idx, ground_truth_image_dir, centernet2_label_dir, retinanet_label_dir, yolov4_label_dir, \
                                 ensemble_label_dir, json_file_name, threshold)
                else:
